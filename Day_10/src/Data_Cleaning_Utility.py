...
class BaseExtractor(ABC):
    @abstractmethod 
    def extract(self)->list[str]|pd.DataFrame:
        pass
...
class DirectoryExtractor(BaseExtractor):
    ...
    def extract(self) -> list[str]:
        files= glob.glob(self._folder_path)
        if not files:
            raise CSVDataError("No CSV files found")
        return files
...
class Pipeline:
    def __init__(self,
                 ...
                 extractor:type[CSVExtractor],
                 cleaners:list[Callable[[pd.DataFrame], pd.DataFrame]],
                 validator:type[DataValidator],
                 profiler:type[DataProfiler],
                 logger:BaseLogger)->None:
        self._dir_extractor=dir_extractor
    ...
    def run(self)->None:
        try:
            dfs=[]
            csv_files = self._dir_extractor.extract()
            for file in csv_files:
                logging.info("===== Operation on %s =====", file)
                data=self._extractor(file).extract()
            ...
            if not dfs:
                raise CSVDataError("No valid datasets processed")
            final_df = pd.concat(dfs, ignore_index=True)
            
            profile_res = self._profiler.profile(final_df)
            self._logger.log_rows(profile_res)
            final_res=final_df.groupby('department',as_index=False,sort=False)['salary'].mean()
            logging.info(f'\n ==========Final Result==========\n\n{final_res}')
...
def main():
    dir_extractor=DirectoryExtractor(FILE_PATH)
    logger=ConsoleLogger()
    cleaners:list[Callable[[pd.DataFrame], pd.DataFrame]]=[DataCleaner.strip_spaces,
              DataCleaner.remove_duplicates,
              DataCleaner.fill_missing_salary,
              DataCleaner.fill_missing_department]
    pipeline=Pipeline(dir_extractor,CSVExtractor,cleaners,DataValidator,DataProfiler,logger)
    pipeline.run()
